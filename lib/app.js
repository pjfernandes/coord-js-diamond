  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      if (data.features.length > 0) {
        ...
        results.innerHTML = `<p><i class="fas fa-map-marker-alt text-primary"></i> <strong>Lat:</strong> ${convert(coordsArray[1],"lat")}</p> <p><i class="fas fa-map-marker-alt text-primary"></i> <strong>Long:</strong> ${convert(coordsArray[0],"long")}</p>`;
...
function add_marker(event) {
  ...
  const results = document.querySelector("p");
  results.innerHTML = `<p><i class="fas fa-map-marker-alt text-primary"></i> <strong>Lat:</strong> ${convert(coordinates.lat,"lat")}</p> <p><i class="fas fa-map-marker-alt text-primary"></i> <strong>Long:</strong> ${convert(coordinates.lng,"long")}</p>`;
